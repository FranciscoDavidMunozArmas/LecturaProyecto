...
export const getCertificatesByUser = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const queryDoc = query(certificatesReference, where("owner", "==", id));
        const dataDocs = await getDocs(queryDoc);
        const data: Certificate[] = dataDocs.docs.map(doc => certificateConverter.fromJSON(doc.data()));
        if(data.length >= 0) {
